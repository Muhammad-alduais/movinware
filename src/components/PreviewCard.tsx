    ...
    >
      <div className="flex items-start mb-6">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-xl font-medium text-gray-900 mb-2 pr-2 rtl:pr-0 rtl:pl-2 text-left rtl:text-right">{title}</h3>
          {(timeline || metric) && (
            <div className="mb-3">
          ...
          <p className="text-gray-600 text-sm leading-relaxed line-clamp-2 text-left rtl:text-right">{description}</p>
        </div>
      </div>
      
      <button 
        onClick={onLearnMore}
        className="flex items-center text-pulse-500 font-medium text-sm group-hover:text-pulse-600 transition-colors duration-300 hover:gap-3 gap-2 rtl:flex-row-reverse"
      >
